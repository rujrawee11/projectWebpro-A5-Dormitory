          ...
          <div class="card">
            <div class="card-image">
              <figure class="image is-4by3">
                <img :src="'http://localhost:5000'+image.file_path" alt="Placeholder image" />
              </figure>
            </div>
...
export default {
  ...
  created() {
    axios
      .get(`http://localhost:5000/blogs/${this.$route.params.id}`)
      .then((response) => {
  ...
  methods: {
    ...
    updateMainImage() {
      ...
      axios
        .put(
          `http://localhost:5000/image/setmain/${this.$route.params.id}/${this.selectMainId}`
        )
        .then((response) => {
    ...
    deleteCurrentImage(imageId) {
      ...
      if (comfirmDeleteImage == true) {
        axios
          .delete("http://localhost:5000/image/" + imageId)
          .then((response) => {
    ...
    submitBlog() {
      ...
      axios
        .put("http://localhost:5000/blogs/" + this.$route.params.id, formData)
        .then((res) => {
          console.log(res);
          this.$router.push({ path: "/" });
        })
